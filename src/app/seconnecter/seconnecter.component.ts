...
export class SeconnecterComponent implements OnInit {
  ...
  async LoginUser(ev: any){
    ...
    ev.preventDefault();
      ...
      let resp = this.LoginService.Ucl(this.Luser);
      resp.subscribe((UserData) => this.LoginError = UserData);
      setTimeout(() => {
        console.log(this.LoginError);
    if( ( this.LoginError != 0) && ( this.LoginError !=  undefined)) {
      this.SessionManager.setItem("UserEmail",this.Luser.email);
      this.SessionManager.setItem("UserID",this.LoginError);

      this.sessioname = this.SessionManager.getItem("UserEmail");
      this.sessioname = this.SessionManager.getItem("UserID");
...
function save(arg0: string, email: string) {
